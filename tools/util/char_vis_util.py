...
def vis_pose(pose, char_model, key_body_ids=None, plot_text=True):
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax = plot_pose(
        fig, ax, pose, char_model, key_body_ids=key_body_ids, plot_text=plot_text
    )
...
def vis_compare_anim(
    ...
    speedup = int(speedup)
    ax, src_ani_obj = plot_ani(
        fig, ax, src_body_pos[::speedup], src_parent_indices, colors="r"
    )
    ax, tgt_ani_obj = plot_ani(
        fig, ax, tgt_body_pos[::speedup], tgt_parent_indices, colors="b"
    )
...
def plot_pose(
    fig, ax, t_pose, char_model, key_body_ids=None, plot_text=True, color="r"
):
    import tools.retargeter.retargeter as retagreter

    root_pos, root_rot_quat, joint_rot_quat = retagreter.extract_frame_data(
        t_pose, char_model
    )
    body_pos, body_rot = char_model.forward_kinematics(
        root_pos, root_rot_quat, joint_rot_quat
    )
    ...
    for i in range(num_jnt):
        if parents[i] != -1:
            ax.plot(
                [body_pos[i, 0], body_pos[parents[i], 0]],
                [body_pos[i, 1], body_pos[parents[i], 1]],
                [body_pos[i, 2], body_pos[parents[i], 2]],
                color=color,
            )

    if key_body_ids is None:
        ax.scatter(
            body_pos[:, 0], body_pos[:, 1], body_pos[:, 2], color=color, marker="o"
        )
